...
async def on_message(message):
    try:
        if message.author == bot.user:
            return
    #if message.guild.id == cursor.execute("SELECT server_id FROM users WHERE id = {}".format(message.author.id)):
    #    return
...
async def leaderboard(ctx):
    embed1 = discord.Embed(color = 0x008B8B, title = 'Топ просвещенных по уровню')
    counter = 0
    ...
    #------------------------------------------------------------------------------------------------------------------
    embed2 = discord.Embed(color = 0x52cc00, title = 'Топ просвещенных по богатству')
    counter = 0
...
async def balance(ctx, member: discord.Member = None):
    if member is None:
        await ctx.send(embed = discord.Embed(
            description = f"""Баланс прогрессивного **{ctx.author.mention}** составляет **{cursor.execute("SELECT cash FROM users WHERE id = {}".format(ctx.author.id)).fetchone()[0]} :dollar:** """
            ))
    else:
        await ctx.send(embed = discord.Embed(
            description = f"""Баланс прогрессивного **{member.mention}** составляет **{cursor.execute("SELECT cash FROM users WHERE id = {}".format(member.id)).fetchone()[0]}:dollar:**  """
            ))
...
async def award(ctx, member: discord.Member = None, amount: int = None):
    if member is None:
        await ctx.send(f"**{ctx.author.mention}**, укажите прогрессивного, которого хотите одарить деньгами")
    else:
        if amount is None:
            await ctx.send(f"**{ctx.author.mention}**, укажи сумму денег, которой хочешь одарить прогрессивного")
        elif amount < 1:
            await ctx.send(f"**{ctx.author.mention}**, укажи корректную сумму денег")
        else:
...
async def take(ctx, member: discord.Member = None, amount = None):
    ...
    #print(cash) 
    if member is None:
        await ctx.send(f"**{ctx.author.mention}**, укажи прогрессивного, у которого хотешь забрать деньги")
    else:
        if amount is None:
            await ctx.send(f"**{ctx.author.mention}**, укажи сумму денег, которую хотешь забрать")
        elif amount == 'all':
...
async def pay(ctx, member: discord.Member = None, amount: int = None):
    ...
    if member is None:
        ...
        if amount is None:
            ...
            await ctx.send(f"**{ctx.author}**, у тебя нет столько денег")
        else:
            cursor.execute("UPDATE users SET cash = cash + {} WHERE id = {}".format(amount, member.id))
...
async def add_shop(ctx, role: discord.Role = None, cost: int = None):
    if role is None:
        await ctx.send(f"**{ctx.author}**, укажи роль, которую ты хочешь добавить в магазин")
    elif role in ctx.author.roles:
        await ctx.send(f"**{ctx.author}**, такая роль уже есть в магазине")
    elif cost is None:
        await ctx.send(f"**{ctx.author}**, укажи стоимость роли, которую ты хочешь добавить в магазин")
    elif cost < 1:
        await ctx.send(f"**{ctx.author}**, укажи корректную стоимость роли")
    else:
        flag = 0
...
async def remove_shop(ctx, role: discord.Role = None):
    if role is None:
        await ctx.send(f"**{ctx.author}**, укажи роль, которую ты хотешь убрать из магазина")
    else:
...
async def buy_role(ctx, role: discord.Role = None):
    if role is None:
        await ctx.send(f"**{ctx.author}**, укажи роль, которую ты хочешь купить в магазине")
    elif role in ctx.author.roles:
        await ctx.send(f"**{ctx.author}**, зачем тебе 2 одинаковые роли? Одумайся")
    elif cursor.execute("SELECT cost FROM shop WHERE role_id = {}".format(role.id)).fetchone()[0] > cursor.execute("SELECT cash FROM users WHERE id = {}".format(ctx.author.id)).fetchone()[0]:
        await ctx.send(f"**{ctx.author}**, у тебя недостаточно денег")
    else:
...
#    channel = bot.get_channel(796094319115239473) #id канала
#    await channel.send(embed = discord.Embed(description = f'`{member.name}` присоединился к числу прогрессивных людей! Поздравляем!', 
#                                             color = 0x008B8B))
...
async def upd(ctx):
    ...
    embed.add_field (name = '1)', value = 'Он про меня забыл 😢', inline = False)
    await ctx.send(embed = embed)
...
async def jpg2(ctx, text):
    ...
    try: 
        ...
        embed = discord.Embed(color = 0xff9900, title = text) # Создание Embed'a
...
async def net_prav30(ctx, error):
    if isinstance(error, commands.MissingPermissions):
        await ctx.send(f'{ctx.author.mention}, лол, у тебя нет прав')
...
async def clear_error2(ctx, error):
    if isinstance(error, commands.MissingRequiredArgument):
        await ctx.send(f'{ctx.author.mention}, укажи, сколько сообщений надо удалить')
